"use strict";
    ...
    function addEnemy() {
      ...
      const type = roll < 0.22 ? "lowWall" : roll < 0.40 ? "highWall" : roll < 0.88 ? "lava" : "smallCat";
      const r = type === "smallCat" ? randomBetween(24, 33) : randomBetween(54, Math.max(68, Math.min(92, width * 0.09)));
      const speed = getWorldSpeed() + randomBetween(35, 95);
      const groundY = getGroundY();
      const w = type === "lava" ? r * randomBetween(4.5, 6.0) : type === "highWall" ? r * 2.55 : type === "smallCat" ? r * 2.25 : r * randomBetween(1.65, 2.2);
      const h = type === "lava" ? r * 0.52 : type === "highWall" ? r * 1.48 : type === "smallCat" ? r * 1.18 : r * randomBetween(2.2, 2.75);
      game.enemies.push({
        type,
    ...
    function update(dt) {
      ...
      if (spawnTimer <= 0) {
        addEnemy();
        spawnTimer = Math.max(0.34, 0.74 - game.time * 0.006);
      }
    ...
    function drawLava(enemy) {
      const w = enemy.w;
      const h = enemy.h * 1.65;
      const lip = Math.max(12, enemy.r * 0.2);
      ctx.fillStyle = "rgba(0, 0, 0, 0.42)";
      ctx.beginPath();
      ctx.ellipse(0, 11, w * 0.56, 15, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.lineWidth = 7;
      ctx.beginPath();
      ctx.roundRect(-w / 2, -h, w, h + lip, 16);
      ctx.fill();
      ctx.stroke();

      ctx.fillStyle = "#202733";
      ctx.beginPath();
      ctx.roundRect(-w / 2 + lip, -h + lip, w - lip * 2, h - lip * 0.3, 13);
      ctx.fill();

      const lavaTop = -h * 0.42;
      const lavaGradient = ctx.createLinearGradient(0, lavaTop - 18, 0, 8);
      ...
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.roundRect(-w / 2 + lip * 1.45, lavaTop, w - lip * 2.9, h * 0.43, 12);
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = "#fff8df";
      ...
      ctx.fillText("JUMP THE GAP", 0, -h * 0.75);
