"use strict";
    ...
    function addEnemy() {
      const r = randomBetween(30, Math.max(42, Math.min(62, width * 0.064)));
      ...
      game.enemies.push({
        ...
        y: groundY - r * 0.85,
        r,
        w: r * randomBetween(1.0, 1.35),
        h: r * randomBetween(1.45, 1.9),
        vx: -speed,
        vy: 0,
        cracked: false
      });
      ...
      for (let p = game.projectiles.length - 1; p >= 0; p -= 1) {
        ...
        for (let e = game.enemies.length - 1; e >= 0; e -= 1) {
          ...
          if (
            ...
            game.score += 60;
            statusText.textContent = "Barricade weggeblusht!";
            break;
          }
        }
      }
    ...
    function applyPowerup(type) {
      if (type === "cookie") {
        game.player.invincible = Math.max(game.player.invincible, 4.0);
    ...
    function checkEnemyHits() {
      for (let i = game.enemies.length - 1; i >= 0; i -= 1) {
        ...
        if (hit && game.player.invincible > 0) {
