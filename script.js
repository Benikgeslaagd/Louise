"use strict";
    ...
    function addEnemy() {
      ...
      const type = roll < 0.30 ? "lowWall" : roll < 0.54 ? "highWall" : roll < 0.86 ? "lava" : "smallCat";
      const r = type === "smallCat" ? randomBetween(24, 33) : randomBetween(42, Math.max(54, Math.min(76, width * 0.078)));
      const speed = getWorldSpeed() + randomBetween(35, 95);
      const groundY = getGroundY();
      const w = type === "lava" ? r * randomBetween(4.0, 5.4) : type === "highWall" ? r * 2.15 : type === "smallCat" ? r * 2.25 : r * randomBetween(1.25, 1.65);
      const h = type === "lava" ? r * 0.52 : type === "highWall" ? r * 1.32 : type === "smallCat" ? r * 1.18 : r * randomBetween(1.75, 2.15);
      game.enemies.push({
        type,
    ...
    function randomTreatType() {
      const roll = Math.random();
      if (roll < 0.025) return "seafood";
      if (roll < 0.065) return "chocolate";
      if (roll < 0.085) return "cookie";
      return "coin";
    }
    ...
    function update(dt) {
      ...
      if (treatTimer <= 0 && game.treats.length < 1) {
        ...
        treatTimer = randomBetween(3.2, 5.2);
      }
    ...
    function drawLava(enemy) {
      ...
      for (let x = -w * 0.46; x <= w * 0.46; x += 22) {
        const y = -h * 0.55 + Math.sin(game.time * 9 + x * 0.08) * 7;
        if (x === -w * 0.46) ctx.moveTo(x, y);
        else ctx.lineTo(x, y);
      }
      ctx.stroke();
      for (let i = 0; i < 9; i += 1) {
        const x = -w * 0.43 + i * w * 0.107;
        const flameH = 16 + Math.sin(game.time * 8 + i) * 7;
        ctx.fillStyle = i % 2 ? "#ffef5e" : "#ff5d1f";
        ctx.beginPath();
        ctx.moveTo(x, -h * 0.88);
        ctx.quadraticCurveTo(x - 10, -h * 0.35, x, -h * 0.2 + flameH * 0.15);
        ctx.quadraticCurveTo(x + 12, -h * 0.35, x, -h * 0.88 - flameH);
        ctx.fill();
      }
    }
