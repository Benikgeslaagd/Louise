"use strict";
    ...
    const game = {
      ...
      lives: 3,
      time: 0,
      player: { x: 0, y: 0, r: 28, vy: 0, onGround: true, invincible: 0, powerTime: 0, magnetTime: 0 },
      enemies: [],
      treats: []
    };
    ...
    function addEnemy() {
      const r = randomBetween(27, Math.max(38, Math.min(58, width * 0.058)));
      const speed = getWorldSpeed() + randomBetween(35, 95);
      const groundY = getGroundY();
      const isLow = Math.random() < 0.72;
      game.enemies.push({
        x: width + r + randomBetween(0, width * 0.28),
        y: isLow ? groundY - r * 0.75 : groundY - r * 2.45,
        r,
        vx: -speed,
        vy: 0,
    ...
    function randomTreatType() {
      ...
      if (roll < 0.20) return "seafood";
      if (roll < 0.42) return "chocolate";
      if (roll < 0.68) return "cookie";
      return "coin";
    }
    ...
    function update(dt) {
      ...
      if (spawnTimer <= 0) {
        addEnemy();
        spawnTimer = Math.max(0.54, 1.18 - game.time * 0.01);
      }

      if (treatTimer <= 0 || game.treats.length < 4) {
        addTreat(randomTreatType());
        treatTimer = randomBetween(0.55, 1.0);
      }
    ...
    function checkEnemyHits() {
      ...
      for (let i = game.enemies.length - 1; i >= 0; i -= 1) {
        ...
        const playerHit = {
          ...
          y: game.player.y - game.player.r * 0.95,
          w: game.player.r * 1.65,
          h: game.player.r * 3.2
        };
        const closestX = Math.max(playerHit.x, Math.min(enemy.x, playerHit.x + playerHit.w));
        const closestY = Math.max(playerHit.y, Math.min(enemy.y, playerHit.y + playerHit.h));
        const dist = Math.hypot(enemy.x - closestX, enemy.y - closestY);
        if (dist < enemy.r * 0.78) {
          game.lives -= 1;
    ...
    function drawBackground() {
      ...
      groundGradient.addColorStop(0, "#29333d");
      groundGradient.addColorStop(1, "#171d24");
      ctx.fillStyle = groundGradient;
      ctx.fillRect(0, roadY, width, roadHeight);
      ctx.fillStyle = "rgba(255, 255, 255, 0.16)";
      ctx.fillRect(0, roadY, width, 5);
      ctx.strokeStyle = "rgba(255, 216, 77, 0.62)";
      ctx.lineWidth = 5;
    ...
    function drawPlayer() {
      ...
      const lean = state === "playing" ? 0.1 : 0;
      const headR = p.r * 0.72;
      const bodyTop = p.r * 0.48;
      const bodyHeight = p.r * 1.38;
      const legY = p.r * 1.92;
      ...
      ctx.moveTo(p.r * 0.48, bodyTop + bodyHeight * 0.22);
      ctx.lineTo(p.r * 1.02 - armSwing, bodyTop + bodyHeight * 0.56);
      ctx.stroke();
